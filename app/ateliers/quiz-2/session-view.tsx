...
import type { Correction, PublicQuestion, Snapshot } from '@/lib/quiz-2/types';
...
function Explanation({ correction, question }: { correction: Correction; question: PublicQuestion }) {
  return <div className="quiz-explanation"><p className="eyebrow">Le repère à retenir</p><h2>Réponse {letters[correction.correct]} — {question.choices[correction.correct]}</h2><p>{correction.explanation}</p><details><summary>Support et sources</summary><p>Diapositives {correction.slides} · Support SSCT initial, périmètre 101 à 226.</p>{correction.sources.length > 0 && <ul>{correction.sources.map(s => <li key={s.url}><a href={s.url} target="_blank" rel="noreferrer">{s.label} ↗</a></li>)}</ul>}</details></div>;
}
...
function AnswerForm({ code, question, selected, connected, onSaved }: { code: string; question: PublicQuestion; selected: number | null; connected: boolean; onSaved: () => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const r = await fetch(`/api/quiz-2/sessions/${code}`, { method: 'POST', signal: AbortSignal.timeout(10000), headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'answer', questionId: question.id, choice }) });
...
export default function SessionView({ code, role }: { code: string; role: 'host' | 'participant' }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      try {
        const r = await fetch(`/api/quiz-2/sessions/${code}?role=${role}`, { cache: 'no-store', signal: timeout.signal });
  ...
  async function act(action: string) {
    ...
    try {
      const r = await fetch(`/api/quiz-2/sessions/${code}`, { method: action === 'delete' ? 'DELETE' : 'POST', signal: AbortSignal.timeout(10000), headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, current: view.current, phase: view.phase }) });
      const data = await r.json(); if (!r.ok) throw new Error(data.error);
      setConfirm(null);
      if (action === 'delete') {
        try { if (localStorage.getItem('quiz2-host') === code) localStorage.removeItem('quiz2-host'); } catch { /* Optional shortcut only. */ }
        router.replace('/ateliers/quiz-2'); return;
      }
  ...
  return <div className={`quiz-session ${host ? 'quiz-host' : 'quiz-participant'}`}>
    <div className="quiz-session-top"><div><p className="eyebrow">Quiz 2 · {host ? 'Espace formateur' : 'Espace participant'}</p><p>Session <b>{code}</b>{view && <> · {view.participants} participant{view.participants > 1 ? 's' : ''} inscrit{view.participants > 1 ? 's' : ''}</>}</p></div><p className="quiz-connection" role="status">{connected ? '● Connecté · Actualisation automatique' : '○ Connexion en cours'}</p></div>
    {error && <div className="quiz-notice" role="alert"><p>{error}</p>{fatal ? <Link href={host ? '/ateliers/quiz-2' : `/ateliers/quiz-2/rejoindre?code=${code}`}>{host ? 'Retour à Quiz 2' : 'Rejoindre à nouveau'}</Link> : <button className="button" onClick={() => setRefresh(n => n+1)}>Réessayer maintenant</button>}</div>}
