import { control, join, newToken, requireHost, snapshot, submit } from '@/lib/quiz-1/engine';
import { bodyOf, failure, json, originOf, rateLimit, setToken, tokenFor } from '@/lib/quiz-1/http';
import { mutateSession, readSession, store } from '@/lib/quiz-1/store';
...
export async function GET(request: Request, context: Context) {
  try {
    ...
    if (new URL(request.url).searchParams.get('export') === '1') {
      ...
      return new Response(csv, { headers: { 'Content-Type': 'text/csv; charset=utf-8', 'Content-Disposition': `attachment; filename="quiz-1-${code}.csv"`, 'Cache-Control': 'no-store, private' } });
    }
    const joinUrl = `${originOf(request)}/ateliers/quiz-1/rejoindre?code=${code}`;
