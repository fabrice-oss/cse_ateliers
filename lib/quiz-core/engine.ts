...
import type { Question, Correction, PublicQuestion, RecordRow, Session, Snapshot } from '../quiz-1/types';
import { QuizError } from '../quiz-1/types';
...
export function createEngine(questions: Question[], quizId: 'quiz-1' | 'quiz-2') {
function newSession(hostToken: string): Session {
  return { version: 1, quizId, code: String(randomInt(100000, 1000000)), hostHash: hash(hostToken), createdAt: Date.now(), expiresAt: Date.now() + lifetime, phase: 'lobby', current: -1, revealed: [], participants: {} };
}
function requireHost(session: Session, token: string) {
...
function requireParticipant(session: Session, token: string) {
...
function join(session: Session, token: string) {
...
function submit(session: Session, token: string, questionId: number, choice: number) {
...
function control(session: Session, token: string, action: string, expectedCurrent: number, expectedPhase: string) {
...
function snapshot(row: RecordRow, role: 'host' | 'participant', token: string): Snapshot {
  const s = row.data;
...
return { newSession, requireHost, requireParticipant, join, submit, control, snapshot };
}
