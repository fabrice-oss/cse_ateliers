...
export const cookieName = (code: string, role: string) => `quiz2_${role}_${code}`;
...
export function setToken(response: NextResponse, code: string, role: string, token: string) {
  response.cookies.set(cookieName(code, role), token, { httpOnly: true, sameSite: 'strict', secure: process.env.NODE_ENV === 'production' && process.env.QUIZ_ALLOW_LOCAL !== '1', path: '/api/quiz-2', maxAge: lifetime / 1000 });
  return response;
}
...
export function failure(error: unknown) {
  ...
  console.error('Quiz 2: opération indisponible', error instanceof Error ? error.name : 'UnknownError');
