...
import { control, hash, join, newSession, newToken, snapshot, submit } from '../lib/quiz-1/engine';
import { questions } from '../lib/quiz-1/questions';
import { QuizError } from '../lib/quiz-1/types';
...
test('25 questions distinctes, une réponse valide, références limitées aux diapositives 1–99', () => {
  assert.equal(questions.length,25);
  questions.forEach((q,i) => { assert.equal(q.id,i+1); assert.equal(q.choices.length,4); assert.equal(new Set(q.choices).size,4); assert.ok(q.correct >=0 && q.correct<4); assert.ok(q.explanation.length > 30); assert.ok(q.slides.match(/\d+/g)!.every(n => +n>=1 && +n<=99)); });
});
...
test('25 questions parcourues, score et résultats finaux corrects pour plusieurs participants', () => {
  ...
  assert.equal(session.phase,'finished'); const first=snapshot({data:session,revision:0},'participant',token),other=snapshot({data:session,revision:0},'participant',second);assert.equal(first.score,25);assert.equal(other.score,0);assert.equal(first.results.length,25);assert.equal(first.participants,2);assert.throws(()=>join(session,newToken()),QuizError);join(session,token);
});
...
test('stockage SQLite durable, révisions concurrentes et expiration', async () => {
  ...
  const dir = await mkdtemp(pathJoin(tmpdir(), 'quiz1-test-'));
  process.env.QUIZ_SQLITE_PATH = pathJoin(dir, 'quiz.sqlite');
  const { store, readSession, mutateSession } = await import('../lib/quiz-1/store');
